...
public class OwnershipScopingEndpointsTests : IClassFixture<ApiWebApplicationFactory>
{
    ...
    public async Task Categories_AreScopedToAuthenticatedUser()
    {
        ...
        var userOneCategories = await userOneClient.GetFromJsonAsync<List<ContentCategory>>("/api/categories");
        var userTwoCategories = await userTwoClient.GetFromJsonAsync<List<ContentCategory>>("/api/categories");
