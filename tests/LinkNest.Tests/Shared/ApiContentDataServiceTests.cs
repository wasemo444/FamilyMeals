...
public class ApiContentDataServiceTests
{
    [Fact]
    public async Task InitializeAsync_LoadsBootstrapSnapshotIntoCache()
    {
        ...
        {
            Categories = [new ContentCategory { Name = "Breakfast" }],
            Settings = new AppSettings { CultureCode = "en" }
        };
        var handler = new FakeHttpMessageHandler()
            .MapGet("/api/bootstrap", bootstrap);
        var service = new ApiContentDataService(new FakeHttpClientFactory(handler));
    ...
    {
        // Arrange
        var created = new ContentCategory { Name = "Lunch" };
        var handler = new FakeHttpMessageHandler()
            .MapGetSequence(
                "/api/bootstrap",
                new AppData(),
    ...
    public async Task ToggleCategoryFavoriteAsync_UpdatesCachedCategoryFromResponse()
    {
        ...
        var handler = new FakeHttpMessageHandler()
            .MapGetSequence(
                ...
                new AppData
                {
                    Categories = [new ContentCategory { Id = categoryId, Name = "Breakfast", IsFavorite = false }]
                },
                new AppData
                {
                    Categories = [new ContentCategory { Id = categoryId, Name = "Breakfast", IsFavorite = true }]
                })
            .MapPost("/api/categories/" + categoryId + "/favorite", _ =>
                new ContentCategory { Id = categoryId, Name = "Breakfast", IsFavorite = true });
        var service = new ApiContentDataService(new FakeHttpClientFactory(handler));
        await service.InitializeAsync();
    ...
    public async Task ArchiveCategoryAsync_ReloadsCacheFromBootstrap()
    {
        ...
        var handler = new FakeHttpMessageHandler()
            .MapGetSequence(
                ...
                new AppData
                {
                    Categories = [new ContentCategory { Id = categoryId, Name = "Breakfast", IsDeleted = false }]
                },
                new AppData
                {
                    Categories = [new ContentCategory { Id = categoryId, Name = "Breakfast", IsDeleted = true }]
                })
