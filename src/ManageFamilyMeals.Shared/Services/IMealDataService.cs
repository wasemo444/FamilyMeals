...
/// Implemented by local persistence (<see cref="MealDataService"/>) and remote API (<see cref="ApiMealDataService"/>).
/// </summary>
public interface IMealDataService
{
    ...
    /// </summary>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <remarks>No-op in <see cref="ApiMealDataService"/>; maintenance runs server-side.</remarks>
    ...
    IReadOnlyList<MealCategory> GetFavoriteCategories(HomeContentFilter filter = HomeContentFilter.All);

    /// <summary>
    /// Returns non-archived categories, optionally filtered by ownership.
    /// </summary>
    /// <param name="filter">Personal, shared, or all categories.</param>
    IReadOnlyList<MealCategory> GetActiveCategories(HomeContentFilter filter = HomeContentFilter.All);
    ...
    Task<MealCategory> AddCategoryAsync(
        string name,
        ContentOwner owner,
